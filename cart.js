...
function generateCartItems(cartItems){
    ...
    cartItems.forEach((item)=>{
        ...
        <div class="cart-item-details flex-grow">
            <div class="cart-item-title font-bold text-sm text-gray-600">${item.make}</div>
            <div class="cart-item-brand text-gray-400 text-sm">${item.name}</div>
        </div>
        <div class="cart-item-counter w-48 flex items-center">
            <div data-id=${item.id} class="chevron-left text-gray-400 bg-gray-100 rounded h-6 w-6 flex justify-center items-center cursor-pointer hover:bg-gray-200 mr-2">
                <i class="fas fa-chevron-left fa-xs"></i>
            </div>
            <h4 class="text-gray-400">${item.quantity}</h4>
            <div data-id=${item.id} class="chevron-right text-gray-400 bg-gray-100 rounded h-6 w-6 flex justify-center items-center cursor-pointer hover:bg-gray-200 ml-2">
